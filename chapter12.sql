...
SELECT round(calcs.average, 0) as average, --rounds average value to zero decimal places
       calcs.median, --median value pulled from subquery
       round(calcs.average - calcs.median, 0) AS median_average_diff --calculate difference between average and median
FROM (
     SELECT avg(p0010001) AS average, --get the average
            percentile_cont(.5)
                WITHIN GROUP (ORDER BY p0010001)::numeric(10,1) AS median --calculates the median, and casts it to a numeric type with one decimal place
     FROM us_counties_2010
     )
AS calcs; --subquery name
...
--finds the employees that have no corresponding values from retirees
SELECT first_name, last_name
FROM employees
WHERE NOT EXISTS (
SELECT id
...
-- creating a temporary table using CTE
...
FROM '/Users/shadowsgeneraldealer/YourDirectory/ice_cream_survey.csv'
...
FROM '/Users/shadowsgeneraldealer/YourDirectory/temperature_readings.csv'
WITH (FORMAT CSV, HEADER);
